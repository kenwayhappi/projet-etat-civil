...
@section('content')
<div class="container">
    <h1 class="page-title">Actes de Mariage</h1>
    <p class="page-subtitle">Gérez les actes de mariage de votre centre</p>

    <!-- Bouton pour ajouter un acte -->
    <div class="mb-4">
        <button class="btn-modern btn-primary-modern" data-bs-toggle="modal" data-bs-target="#addMarriageModal">
            <i class="fas fa-plus me-2"></i> Ajouter un Acte de Mariage
        </button>
    </div>
        ...
        <tbody id="acts-table">
            @foreach ($acts as $act)
                <tr data-id="{{ $act->id }}">
                    ...
                    <td>{{ $act->details['marriage_place'] ?? '-' }}</td>
                    <td>
    ...
    <div class="modal fade" id="addMarriageModal" tabindex="-1" aria-labelledby="addMarriageModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addMarriageModalLabel">Ajouter un Acte de Mariage</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="marriageForm" action="{{ route('agent.acts.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            <label for="marriage_date" class="form-label">Date de Mariage <span class="text-danger">*</span></label>
                            <input type="date" class="form-control" id="marriage_date" name="details[marriage_date]" required>
                            <div id="marriage_date_error" class="text-danger mt-1"></div>
                        </div>
                        <div class="mb-3">
                            <label for="marriage_place" class="form-label">Lieu de Mariage <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="marriage_place" name="details[marriage_place]" required>
                            <div id="marriage_place_error" class="text-danger mt-1"></div>
                        </div>
    ...
    <div class="modal fade" id="editMarriageModal" tabindex="-1" aria-labelledby="editMarriageModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editMarriageModalLabel">Modifier un Acte de Mariage</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editMarriageForm" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            <label for="edit_marriage_date" class="form-label">Date de Mariage <span class="text-danger">*</span></label>
                            <input type="date" class="form-control" id="edit_marriage_date" name="details[marriage_date]" required>
                            <div id="edit_marriage_date_error" class="text-danger mt-1"></div>
                        </div>
                        <div class="mb-3">
                            <label for="edit_marriage_place" class="form-label">Lieu de Mariage <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="edit_marriage_place" name="details[marriage_place]" required>
                            <div id="edit_marriage_place_error" class="text-danger mt-1"></div>
                        </div>
                        <button type="submit" class="btn btn-primary rounded-pill">Enregistrer</button>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const addActModal = new bootstrap.Modal(document.getElementById('addMarriageModal'));
    const editActModal = new bootstrap.Modal(document.getElementById('editMarriageModal'));
    ...
    document.getElementById('marriageForm').addEventListener('submit', function(e) {
        ...
        document.getElementById('marriage_date_error').textContent = '';
        document.getElementById('marriage_place_error').textContent = '';
        ...
        .then(response => {
            ...
            try {
                if (response.status === 201) {
                    ...
                    let formattedDate = '-';
                    if (act.details.marriage_date && typeof moment === 'function') {
                        const parsedDate = moment(act.details.marriage_date);
                        if (!parsedDate.isValid()) {
                            console.warn('Date de mariage invalide:', act.details.marriage_date);
                        } else {
                    ...
                    const row = `
                        <tr data-id="${act.id}">
                            ...
                            <td>${formattedDate}</td>
                            <td>${act.details.marriage_place || '-'}</td>
                            <td><span class="badge bg-${act.status === 'validated' ? 'success' : (act.status === 'pendingrefer' ? 'warning' : 'danger')}">${act.status}</span></td>
                            <td>
                                ${act.status === 'pending' ? `
        ...
        .catch(error => {
            ...
            document.getElementById('marriage_date_error').textContent = errors['details.marriage_date'] ? errors['details.marriage_date'][0] : '';
            document.getElementById('marriage_place_error').textContent = errors['details.marriage_place'] ? errors['details.marriage_place'][0] : '';
    ...
    document.getElementById('acts-table').addEventListener('click', function(e) {
        if (e.target.closest('.edit-act')) {
            ...
            document.getElementById('edit_marriage_date').value = details.marriage_date || '';
            document.getElementById('edit_marriage_place').value = details.marriage_place || '';

            // Effacer les messages d'erreur précédents
            document.getElementById('edit_spouse1_name_error').textContent = '';
            document.getElementById('edit_spouse2_name_error').textContent = '';
            document.getElementById('edit_marriage_date_error').textContent = '';
            document.getElementById('edit_marriage_place_error').textContent = '';
    ...
    document.getElementById('editMarriageForm').addEventListener('submit', function(e) {
        ...
        .catch(error => {
            ...
            document.getElementById('edit_marriage_date_error').textContent = errors['details.marriage_date'] ? errors['details.marriage_date'][0] : '';
            document.getElementById('edit_marriage_place_error').textContent = errors['details.marriage_place'] ? errors['details.marriage_place'][0] : '';
